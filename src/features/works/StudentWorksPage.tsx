  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const worksData: Work[] = Array.isArray(res.data?.items) ? res.data.items : []
        setWorks(worksData)
      } catch (e: any) {
        if (cancelled) return
          ...
          >
            <CardContent className="p-4">
              ...
              <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-muted-foreground">
                ...
                <span className="hidden md:inline">🏢 Корпус {work.corpus}</span>
                <span className="hidden md:inline text-muted-foreground/50">|</span>
                <span className="hidden md:inline">🚪 Ауд. {work.room}</span>
              </div>
            </CardContent>
          </Card>
        ))}
